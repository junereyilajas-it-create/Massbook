...
function MassSchedulePage() {
  ...
  const filteredSlots = useMemo(() => {
    ...
    // Filter by view mode based on event_date
    ...
    if (viewMode === 'today') {
      ...
      filtered = schedule.filter(slot => {
        ...
        return slot.event_date === todayStr;
      ...
      filtered = schedule.filter(slot => {
        ...
        return slot.event_date >= startOfWeekStr && slot.event_date <= endOfWeekStr;
      ...
      filtered = schedule.filter(slot => {
        ...
        return slot.event_date >= startOfMonthStr && slot.event_date <= endOfMonthStr;
  ...
  }, [query, schedule, viewMode, currentMonth]);
  ...
  const getWeekSchedule = () => {
    ...
    return dayNames.map(dayName => {
      ...
      return {
        ...
        schedules: filteredSlots.filter(slot => slot.event_date === dateStr).sort((a, b) => a.time.localeCompare(b.time))
